...
大多数生产部署对出现故障的系统定义了平均修复时间（MTTR），这通常是系统在线恢复将花多长时间的评估。
MaxTTR，是单个服务器的最大故障时间；MTSR，储存或索引集群的最大故障时间。

 2. Flume Channel的容量

Flume Channel的容量在生产数据的应用程序和存储系统直接提供了一个缓冲区。无法访问存储系统的最大时间等于MaxSTR。这意味着在一段时间内，应用程序仍然能生产和写入数据到Flume。计算总缓冲能力很简单。假设所有的应用服务器事件的综合生产能力在高峰小时是每秒Pmax个事件。那么在这个时期生产的总数表示为：
Tmax = Pmax * MaxTSR
为了在整个Flume部署中优异的Channel容量为提高Tmax的25%的容量。

...
UserMemory：Saprk程序产生的临时数据或者是自己维护的一些数据结构也需要给予它一部分的存储空间，可以认为这是程序运行时用户可以主导的空间，叫用户操作空间。它占用的空间是(Java Heap - Reserved Memory) x 25%(默认是25%，可以有参数供调优)，这样设计可以让用户操作时所需要的空间与系统框架运行时所需的空间分离开。

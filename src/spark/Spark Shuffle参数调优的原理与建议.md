...
```
默认的执行内存计算公式：spark.shuffle.memoryFraction(0.2) *  spark.shuffle.safetyFraction(0.8)=0.16  也就是说，是JVM HeaoSize的16% 通过 spark.shuffle.memoryFraction可以调整Spill的触发条件，即Shuffle占用内存的大小，进而调整Spill的频率和GC的行为。
